...
;module.exports = {
    ...
    get: function (requestOptions) {
        return jUtility.getJSON(requestOptions);
    }, // end fn
    ...
    getJSON: function (requestOptions) {
        ...
        jApp.log('6.5 ajax options set, executing ajax axios request');

        return window.axios
            .get(opts.url, {params: opts.data})
            .then((response) => {
                opts.success(response.data);
                opts.always(response.data);
                opts.complete(response.data);
            }, (error) => {
                opts.fail(error.response.data);
                opts.always(error.response.data);
                opts.complete(error.response.data);
            });
    }, // end fn
