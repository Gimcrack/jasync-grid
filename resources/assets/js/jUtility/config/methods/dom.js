...
export default {
    ...
    DOM: {
        ...
        initScrollbar: function () {
            let container = document.querySelector('.table-grid');
            jUtility.DOM.ps = new PerfectScrollbar(container);
        }, //end fn
        ...
        buildForm: function (params, key, htmlKey, tableFriendly) {
            ...
            if (typeof jApp.aG().html.forms[htmlKey] === 'undefined') return;
        ...
        removeRows: function (all) {
            var identifiers = _.map(jApp.aG().dataGrid.data, jApp.opts().pkey);
        ...
        updateGrid: function () {
            ...
            $.each(jApp.activeGrid.dataGrid.delta, function (i, oRow) {
                ...
                if (!tr.length) {
                    tr = $('<div/>', {
                        'class': 'table-row',
                        'data-identifier': oRow[jApp.opts().pkey],
                        'data-jsonkey': i
                    });
                    jApp.log('couldnt find tr, making a new one', tr);
                    appendTR = true;
                ...
                if (appendTR) {
                    jApp.log('adding the row to the dom', $('#tbl_grid_body').append(tr));
            ...
            if (typeof params[0] === 'object') { // determine if button is a dropdown menu
                ...
                if (params.length > 1) {
                    ...
                    _.each(params, function (o, key) {
                        ...
                        $btn_choice = $('<a/>', $.extend(true, {'data-permission': ''}, _.omit(o, 'fn'), {
                            href: '#',
